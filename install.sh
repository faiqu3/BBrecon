...
gotools["goaltdns"]="go install github.com/subfinder/goaltdns@latest"
...
gotools["httprobe"]="go install github.com/tomnomnom/httprobe@master"
...
eval git clone https://github.com/FortyNorthSecurity/EyeWitness.git ~/EyeWitness $DEBUG_STD
eval git clone https://github.com/geeknik/the-nuclei-templates.git ~/nuclei-templates/extra_templates $DEBUG_STD
eval wget -q -O - https://raw.githubusercontent.com/haccer/subjack/master/fingerprints.json > ${tools}/fingerprints.json $DEBUG_STD
...
eval $SUDO bash ~/EyeWitness/Python/setup/setup.sh $DEBUG_STD
...
wget -q -O - https://raw.githubusercontent.com/subfinder/goaltdns/master/words.txt > ${goaltdns}
